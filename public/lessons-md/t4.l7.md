...
## Floating point, compressed into one paragraph
...
```c
w ≈ scale × q          q ∈ [-127, 127] (int8) or [-7, 7] (int4)
...
## How production quantization actually works
...
- **×2** — FP8 decode rate (Half the weight bytes per token on a bandwidth-bound path ≈ twice the tokens/s.)
- **×4** — INT4 capacity (A 70B model: 140 GB → ~35 GB. One GPU instead of four.)
- **~1%** — INT4 quality cost (Per-group GPTQ/AWQ with calibration, typical benchmarks.)
- **128** — group size (The sweet spot: fine enough scales, negligible metadata overhead.)

---

> **[analogy]** Quantization is your **lossy codec decision** — Opus vs FLAC: pick the bitrate where the artifact is inaudible *for this content*. The calibration set is your listening test: GPTQ/AWQ run a few hundred representative samples through and adjust scales/rounding so the error hides where the model doesn't care (second-order methods, literally approximating which weights matter). And per-group scales are your **per-frame normalization** in audio: local loudness, not one global gain.

---

> **[warning]** The cliffs are real and two-sided. **(1) Outliers:** a handful of huge-magnitude weights/activations wreck a shared scale — the reason per-group and "keep outliers in FP16" (AWQ, LLM.int8()) exist. **(2) Activations are harder than weights:** weight-only INT4 is gentle; *activation* INT4 is where accuracy falls off a cliff on many models — hence FP8 for activations (range!) rather than INT4. Rule of thumb: weights tolerate 4 bits with care; activations want 8; the KV cache sits between.
...
**Exercise: The interactive quantizer · AWQ-style calibration challenge**

1. Type 65537.0: watch FP16 overflow (inf) while BF16 survives — range vs precision in one number.
2. Type 3.14159265: compare mantissa loss across FP16/BF16/FP8-E4M3.
3. Select INT4, then run the 256-value tensor with one per-tensor scale; record its max absolute error and drift.
4. Without changing the tensor, run group-128 next; compare both metrics. The guided task completes only after both runs.
5. Inject the salient 47.3 outlier, then shrink the group until drift recovers. This is the calibration decision behind AWQ-style outlier protection, made visible.

_You now own the serving engineer's quantization intuition: bits buy precision/range, groups buy error locality, calibration buys accuracy — and on a bandwidth-bound decode loop, every bit saved per weight is literal throughput._

---

**Q1. Quantization speeds up LLM decode primarily because…**
   A. Smaller numbers compute faster on ALUs
   B. Decode is bandwidth-bound: fewer bytes per weight = more tokens/s through the same HBM, plus more model fits in memory
   C. It removes the softmax
   D. It reduces the number of layers
   Answer: B — The roofline argument (T4.L3): on the bandwidth slope, halving bytes per token doubles the rate. Compute-bound paths (prefill) only speed up with formats the tensor cores run natively faster (FP8).

**Q2. BF16 exists because deep learning values…**
   A. More mantissa bits than FP16
   B. FP32's exponent RANGE over mantissa precision — activations/gradients span magnitudes, and overflow is worse than rounding
   C. Exact integer arithmetic
   D. Compatibility with FP64 hardware
   Answer: B — 8 exponent bits (like FP32) + 7 mantissa bits. Two significant digits that never overflow beat three that do — the format trade in one sentence.

**Q3. Per-group scales (group size 128) improve weight quantization because…**
   A. They reduce metadata
   B. Each group's scale hugs its local absmax, shrinking the per-weight error bound (scale/2) by roughly an order of magnitude for ~4 bits/weight of metadata
   C. They allow negative weights
   D. They eliminate the need for calibration
   Answer: B — Error ≤ scale/2; one global scale is hostage to the tensor's biggest outlier. Local scales localize the damage — the single most effective quality knob in GPTQ/AWQ-class methods.

**Q4. The standard quality ordering for what to quantize hardest is…**
   A. Activations → weights → KV cache
   B. Weights (4-bit OK with care) → KV cache (8-bit) → activations (prefer FP8 over INT4; accuracy cliff below)
   C. Everything to INT4 equally
   D. Nothing — quantization is always lossy
   Answer: B — Weights tolerate aggressive quantization; activations are the fragile path (outliers, range) and want 8-bit with good range; KV sits between. Hence: INT4 weights + FP8 activations + FP8/INT8 KV as the common production mix.
