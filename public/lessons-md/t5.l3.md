...
- **∝ prompt × params** — TTFT driver (Prefill FLOPs scale with prompt length — compute-bound, tensor cores busy.)
- **∝ bytes/token** — ITL driver (Weight + KV reads per step ÷ HBM bandwidth — the roofline slope.)
- **<100 ms** — ITL comfort (Streaming feels responsive; human reading speed is ~50 ms/token.)
- **goodput** — the real KPI (Throughput within SLO. Everything else is marketing.)

---

## The tension: batching buys throughput, costs latency
...
> **[analogy]** You run this exact split in every web service: **prefill is the cold-start/DB query** (one expensive bounded operation before the first byte — compute-bound), **decode is the streaming response** (per-chunk cost dominated by I/O, here HBM instead of network). TTFT is your TTFB; ITL is your chunk period; and "goodput within SLO" is your p99-gated capacity planning. The seesaw is your thread-pool tuning: bigger pools (batches) raise throughput until queueing latency eats the gain. Same curve, hotter metal.

---

> **[info]** A quick estimation kit. TTFT ≈ queue + (prompt_tokens × 2 × params) / effective_FLOPs. ITL ≈ (weight_bytes + KV_bytes_per_token × batch) / bandwidth / batch … i.e., per-token time stays ~flat while throughput × batch — until the compute roof interrupts. These two lines get you within ~2× of any real system, which is enough to kill bad capacity plans in the meeting room instead of the postmortem.

---

## Why inference is *not* "training, but smaller"
...
**Q1. TTFT and ITL are dominated by different physics because…**
   A. They run on different models
   B. Prefill is compute-bound (FLOPs ∝ prompt length, tensor-core limited) while decode is bandwidth-bound (weight+KV bytes per step ÷ HBM bandwidth)
   C. TTFT is network latency only
   D. ITL depends on the tokenizer
   Answer: B — Two roofline regimes: prefill sits at the compute roof, decode on the bandwidth slope. Hence separate metrics, separate optimizations, and increasingly separate GPUs (disaggregation).

**Q2. Goodput is defined as…**
   A. Total tokens/s under any conditions
   B. Throughput subject to SLO constraints (e.g. TTFT < 2 s, ITL < 100 ms) — the honest capacity metric
   C. GPU utilization percentage
   D. Requests that return 200 OK
   Answer: B — Raw throughput can always be bought by sacrificing latency; goodput measures what you can serve while keeping the user-facing promises. Capacity reviews should speak goodput.

**Q3. The batching seesaw between TTFT/ITL and throughput exists because…**
   A. Batches require padding
   B. Larger batches amortize weight reads (throughput ↑) but read more KV per step and deepen queues (per-token latency ↑)
   C. GPUs throttle at high utilization
   D. Tokenizers slow down in batches
   Answer: B — Same trade as thread-pool sizing: utilization rises with concurrency until queueing and per-step work eat the latency budget. Engines pick a point on the curve; SLOs pick it for you.

**Q4. Decode cannot simply be "parallelized away" like training because…**
   A. GPUs are busy
   B. Autoregression is inherently serial: token t+1 depends on token t — you can only make steps cheaper (quantization), amortized (batching), or fewer (speculative decoding)
   C. The KV cache is read-only
   D. Softmax is sequential
   Answer: B — Training sees the full sequence in parallel; inference must generate it in order. Every decode optimization is one of those three shapes — a useful taxonomy for reading new papers.
