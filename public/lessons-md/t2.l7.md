...
## §1–2: The problem, in OS vocabulary
...
> **[isomorphism]** Read the paper's Figure 2 (memory waste breakdown) next to your T1 fragmentation simulator output: "reserved-but-unused" is internal fragmentation from worst-case allocation; "unusable gaps" is external fragmentation from variable sizes. The paper never says "allocator," but §2 is an allocator autopsy.

---

## §3: PagedAttention — paging, verbatim
...
- **page / frame** (Fixed 4 KB units of virtual/physical memory; any frame backs any page.) ≡ **KV block** (Fixed 16-token units of logical/physical KV; any physical block serves any sequence.)
- **page table + MMU** (Per-process indirection; hardware translates on every access.) ≡ **block table + kernel** (Per-sequence indirection; the PagedAttention CUDA kernel translates per block.)
- **fork() + COW** (Children share frames read-only; writes copy the touched frame.) ≡ **beam/sample sharing** (Branches share prompt blocks read-only; diverging writes copy one block.)
- **swap + eviction** (Cold frames to disk under pressure; refault on access.) ≡ **preemption: swap / recompute** (Victim sequences' blocks to CPU RAM — or discarded and recomputed on resume.)

---

## §4: Scheduling — the T2.L4 layer
...
T2.L2's 4-level radix tree exists because a 48-bit address space is astronomically sparse. A sequence is small (≤ ~1M tokens → ≤ 65k blocks), so a **flat array** block table is tiny (65k × 8 B = 512 KB worst case) and translation is a single indexed load. Design rule from both worlds: match the table depth to the sparsity of the space. Later systems (e.g. some TGI/TRT-LLM modes) use the same flat scheme; the radix tree returns when the "address space" is a whole cluster's KV pool — see T5.L8 on Mooncake's distributed KV.
...
**Q1. The three KV-memory wastes the paper diagnoses in prior systems map to which allocator phenomena?**
   A. Leaks, races, deadlocks
   B. Internal fragmentation (worst-case reservation), external fragmentation (variable segments), and duplication (unshared prefixes)
   C. Thrashing, false sharing, TLB misses
   D. Stack overflow, heap overflow, double-free
   Answer: B — Reserved-but-unused slots are internal fragmentation; unusable gaps between variable-size segments are external; re-copied prompts for parallel samples are duplication that page-style sharing eliminates. §2 is an allocator autopsy.

**Q2. PagedAttention bounds KV waste to <4% primarily because…**
   A. It compresses KV tensors to FP8
   B. Fixed-size token blocks make external fragmentation impossible and cap internal waste at the partially filled tail block
   C. It evicts cold sequences to disk
   D. It shares weights across GPUs
   Answer: B — The fixed-block maneuver from T1.L4: any free block fits any sequence (no external fragmentation), and waste is ≤ block_size−1 tokens in the tail — the same physics as OS page frames.

**Q3. Beam search / parallel sampling in vLLM shares memory exactly like…**
   A. A RAID array
   B. fork() with copy-on-write: branches share the prompt's physical blocks with refcounts; a diverging write copies just that block
   C. An mmap'd read-only file
   D. A spinlock-protected queue
   Answer: B — Shared blocks are read-only across branches; when one branch generates into a shared block, the manager copies it (COW) and decrements the source refcount. The paper's Figure 4 is the fork() diagram.

**Q4. The swap-vs-recompute preemption debate in §4 mirrors the OS choice between…**
   A. Spinlocks and mutexes
   B. Swapping anonymous pages to disk versus dropping clean file-backed pages for later re-read — bandwidth vs recompute
   C. Huge pages and base pages
   D. CFS and deadline scheduling
   Answer: B — Swapping KV to CPU RAM costs PCIe bandwidth but preserves state; recomputation frees HBM but repays prefill FLOPs. The OS makes the identical call for anonymous vs file-backed pages.

**Q5. Why does the block-table indirection cost so little relative to its benefit?**
   A. GPUs cannot detect the indirection
   B. The per-access translation overhead (indexed block lookups) is small next to the 2–4× batch-size gain from reclaimed memory — memory, not compute, was the bottleneck
   C. The block table is stored in L1 cache permanently
   D. Indirection is free in CUDA
   Answer: B — Decode is memory-capacity- and bandwidth-bound: reclaiming 60–80% of KV space multiplies batch size, which multiplies throughput. A few extra index loads per block are noise against that win — the MMU trade, reprised.
