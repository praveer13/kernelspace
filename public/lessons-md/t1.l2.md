# T1.L2 — Pointers & Manual Memory

_Track T1: C-Level Mental Model · ~25 min · kernelspace_

> The memory-grid visualizer: dereference, address arithmetic, and cause a real segfault — safely.
...
## The four operations — that's all of it
...
## The segfault, demystified forever
...
> **[analogy]** Java and Python give you pointers with the dangerous bits filed off: a **reference** is a pointer the runtime owns. It dereferences automatically, forbids arithmetic, and guarantees non-null-or-NPE. The JVM even uses **compressed oops** — 32-bit pointers on a 64-bit heap — to save cache space, which tells you how much pointer *traffic* matters. When you got a NullPointerException, you were segfaulting politely: same null address, but the runtime checks instead of the MMU.

---

> **[segfault]** "Undefined behavior means it might crash." Worse: it might *work* — today, on this compiler, at -O0 — and corrupt data silently at -O2 next quarter. Reading past an array often returns plausible garbage because the memory is mapped and stale. The segfault is the *lucky* outcome; silent corruption is the expensive one. Sanitizers (ASan) exist to make the unlucky case loud.

---

_pointers all the way down_

- **pointer + offset arithmetic** (a[i] ≡ *(a+i): address = base + index × size. Hardware-checked by the MMU.) ≡ **KV block table entry** (token t's KV lives at block_table[t/B] + (t%B) × stride — same arithmetic, GPU side.)
- **NULL dereference → SIGSEGV** (Unmapped address, MMU page fault, kernel kills the process.) ≡ **invalid block id → CUDA fault** (A corrupt block table faults on-device; the serving process dies just as dead.)

---

## Manual memory: the ownership question
...
**Exercise: Pointer lab: arithmetic, indirection, and faults**

1. Allocate `long vals[4]` on the grid; read `*(p+2)` and confirm it lands exactly 16 bytes along.
2. Build a two-level pointer (`long **pp`) and follow both hops on the grid.
3. Walk one element past the array; inspect the stale bytes you read (mapped ≠ valid).
4. Dereference `NULL` and read the fault path: MMU → kernel → SIGSEGV → core dumped.

_The grid makes the key point visible: memory is a flat array of bytes, and a pointer is just an index into it. The two-hop chase (pp → p → value) is exactly how page tables and block tables work — one indirection resolving to another. And the NULL crash was the *hardware* catching you: the OS left page 0 unmapped precisely so that mistake is loud instead of silent._

---

**Q1. If `double *p` holds address 0x1000, what address does `p + 3` hold?**
   A. 0x1003
   B. 0x100C
   C. 0x1018
   D. It depends on the OS
   Answer: C — Pointer arithmetic scales by the pointee size: 3 × 8 bytes = 24 = 0x18. This single rule is how array indexing, struct walking, and KV-block addressing all work.

**Q2. Dereferencing NULL crashes your process because…**
   A. The C runtime checks every pointer against NULL
   B. Address 0 is left unmapped by the OS; the MMU page-faults and the kernel delivers SIGSEGV
   C. The CPU has a special NULL-detection register
   D. The compiler inserts a null check before every load
   Answer: B — No language-level check happens. The first page is deliberately unmapped, so the hardware faults and the OS kills the process. A segfault is a kernel verdict about an address-space violation.

**Q3. Why is reading one element past an array more dangerous than crashing?**
   A. It isn't — it always crashes immediately
   B. The memory is often mapped and stale, so the program continues with silently corrupted data
   C. It always corrupts the heap allocator immediately
   D. The TLB caches the fault
   Answer: B — Undefined behavior's worst case is not the crash — it is the plausible garbage. Adjacent stack/heap bytes are usually mapped, so you read stale data and keep going. ASan exists to convert this into an immediate, loud failure.

**Q4. Java references differ from C pointers in all of the following EXCEPT:**
   A. References forbid arithmetic
   B. References are automatically dereferenced
   C. References are guaranteed non-null by the type system
   D. References are managed addresses the runtime can relocate (compaction)
   Answer: C — Java references can absolutely be null — NPE is the polite segfault, checked by the runtime instead of the MMU. What you lose vs C is arithmetic and control; what you keep is the concept: a managed address.
