...
## Slices: windows into memory you don't own
...
```rust
// A wire parser that allocates NOTHING:
...
```rust
/// A view into a shared-memory slab mapped from /dev/shm.
...
**Q1. A Rust slice &[T] is…**
   A. An owned array on the heap
   B. A fat pointer (ptr, len) describing a borrowed, contiguous view — the compiler proves it never outlives its source
   C. A copy of the original data
   D. A reference-counted buffer
   Answer: B — 16 bytes of metadata, zero payload copies, lifetime-checked. This is what makes zero-copy parsing/serialization the safe default in Rust rather than an expert technique.

**Q2. bytes::Bytes achieves zero-copy sharing by…**
   A. Compressing the buffer
   B. Reference-counting one immutable underlying buffer and handing out (offset, len) slices that just bump the count
   C. Using unsafe everywhere
   D. Copying on first write
   Answer: B — Slicing a Bytes is a refcount bump — no memcpy. It is the tokio/hyper ecosystem's backbone for passing buffers between layers without the serialization tax.

**Q3. Inside an unsafe block, Rust…**
   A. Disables the borrow checker entirely
   B. Permits five extra powers (raw-pointer deref, unsafe/FFI calls, mutable statics, unsafe traits, unions) — normal borrow rules still apply
   C. Turns off bounds checking globally
   D. Skips all lifetime analysis
   Answer: B — unsafe is additive, not a bypass: it unlocks capabilities the compiler cannot verify, which is why invariants move into SAFETY comments and Miri tests around a tiny, wrapped surface.

**Q4. The professional pattern for unsafe Rust is…**
   A. Mark whole modules unsafe for convenience
   B. Tiny unsafe internals behind a safe, borrow-checked API, with documented invariants and Miri coverage
   C. Avoid unsafe in all circumstances, even FFI
   D. Use unsafe only in tests
   Answer: B — Callers should never inherit obligations they can't see. Safe wrappers concentrate risk into auditable units — which is also why Rust's aggregate CVE story beats C++ even though unsafe exists.
