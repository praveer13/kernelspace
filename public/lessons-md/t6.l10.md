# T6.L10 — Security Boundaries for Agentic Inference

_Track T6: Mega-Scale Serving · ~35 min · kernelspace_

> The model emits untrusted instructions, tools touch real systems, shared caches remember prompts, and accelerators process secrets. Secure serving starts by drawing four different boundaries instead of calling the whole stack “the sandbox.”
Agentic inference joins four security problems that ordinary chat could mostly keep separate. **Model output is untrusted input** even when it looks like valid JSON. A tool runner executes code and opens files or sockets. A serving engine retains prompt-derived KV state for reuse. The infrastructure layer handles model weights, tenant data, credentials, and accelerator memory.
...
_fig 1 — deny by default at every agentic boundary_

Components: untrusted input (prompt · upload · tool result) · model server (tenant-scoped KV) · capability broker (schema · policy · audit) · ephemeral microVM (tool + scratch only) · secret broker (short-lived grants)

Steps:
1. Treat prompts, uploads, retrieved text, and prior tool output as hostile data. They may influence the model but receive no authority.
2. The model proposes a typed action. A deterministic broker validates schema, identity, tenant, destination, and policy before anything executes.
3. Mint a short-lived capability for one operation. Never place ambient cloud credentials or the control-plane token inside the worker.
4. Run the tool in a disposable, bounded microVM. Filter egress on the host, collect a capped result, then destroy the environment.

---

## Tool execution: a microVM is a boundary, not a policy
...
## Confidential GPU inference: protect use, not intent
...
## Prefix caches create a tenant-visible timing oracle
...
- **125 ms** — Firecracker minimum reported startup (Application-code startup under favorable conditions; use snapshots and measure your own image and host.)
- **4–8%** — one H100 confidential-inference study (Study-specific throughput overhead. Other configurations report larger gaps, so benchmark before capacity planning.)
- **hit ≠ miss** — cache timing is an observable signal (A shared prefix cache can leak membership even when raw KV memory is never exposed.)
- **0 ambient** — credentials inside a tool worker (Mint one short-lived, operation-scoped capability only after deterministic policy approves the call.)

---

> **[warning]** T6.L8's grammar mask can guarantee that `{"tool":"transfer","amount":1000}` parses. It cannot prove that the user owns the account, that the destination is allowed, or that the model was not prompt-injected. Resolve identity and policy in ordinary deterministic code, bind the decision to a narrow capability, and make the sandbox the final containment layer — never the first approval layer.

---

_One old systems principle at four layers_

- **syscall validation** (User space proposes; the kernel validates arguments and authority.) ≡ **tool-call broker** (The model proposes; deterministic code validates schema, identity, and policy.)
- **process capability** (Authority is an explicit handle, not ambient global access.) ≡ **single-use credential** (One approved operation receives one short-lived, destination-scoped grant.)
- **address-space isolation** (A process cannot infer or read another process by default.) ≡ **tenant cache namespace** (KV and prefix identity include tenant and model context by construction.)

---

> **Field note · Firecracker security design**
> Firecracker project · Living design document · verified 2026-08
>
> Read the threat-containment, jailer, seccomp, and network sections as an exercise in boundary ownership. The most important sentence is the one that refuses responsibility: guest egress filtering belongs to the host. Production security comes from finding every such seam and assigning it to a named control, owner, test, and log.
>
> [Read the primary source](https://github.com/firecracker-microvm/firecracker/blob/main/docs/design.md)

---

**Q1. Why is a container alone a weak boundary for hostile model-generated code?**
   A. Containers cannot set memory limits
   B. The workload still shares the host kernel; a microVM adds a hardware-virtualized guest kernel boundary
   C. Containers cannot run Python
   D. A container always has root on the host
   Answer: B — Namespaces, cgroups, and seccomp remain valuable, but a container reaches one shared host kernel. A minimal KVM microVM adds another containment boundary; the surrounding platform must still constrain authority and egress.

**Q2. Remote attestation for confidential GPU inference proves primarily that…**
   A. The model will never leak data in its output
   B. Expected measured software and confidential-capable hardware are running before a tenant releases secrets
   C. Every tool call is authorized
   D. Inference has zero performance overhead
   Answer: B — Attestation establishes a measured execution environment. Application policy, output safety, metadata protection, and tool authorization remain separate responsibilities.

**Q3. How can a prefix cache leak information without exposing KV tensors?**
   A. The tokenizer prints the prompt
   B. A cache hit and miss have measurably different latency, creating a membership oracle for guessed prefixes
   C. The GPU logs every token publicly
   D. The cache key is always the plaintext prompt
   Answer: B — The attacker observes timing rather than memory. Tenant-scoped, server-derived namespaces are the default defense; sensitive prefixes should not cross tenant boundaries.

**Q4. A model emits a schema-valid request to transfer money. What authorizes execution?**
   A. The JSON grammar
   B. The model confidence score
   C. A deterministic broker that checks identity and policy, then mints one narrow, short-lived capability
   D. The microVM booting successfully
   Answer: C — Parsing establishes syntax only. Authorization belongs to deterministic policy code, and the worker should receive no authority beyond that single approved operation.

---

For one real agent tool, write four negative tests before a happy-path demo: a prompt that asks for an undeclared destination, a tool that forks or emits unbounded output, a guest that probes the metadata endpoint, and a second tenant that times a known sensitive prefix. Record which layer rejects each attempt and which audit event proves it. If two layers both “probably handle it,” the boundary has no owner.
