...
**Q1. WebAssembly's security model is best described as…**
   A. Process isolation managed by the OS
   B. Capability-based: a module touches only its own linear memory and exactly the host functions it was explicitly given
   C. A Java-style SecurityManager policy file
   D. Code signing with trusted publishers
   Answer: B — No ambient authority: no syscalls unless the host imports them, no memory outside the linear buffer. The sandbox is structural, which is why it works for untrusted code in tabs and plugins.

**Q2. Rust is an unusually good Wasm source language because…**
   A. Rust invented WebAssembly
   B. It has no GC/runtime to port — just thin alloc/panic shims — so the same zero-cost code compiles to wasm32 with mature tooling
   C. Rust is interpreted like JavaScript
   D. Wasm only supports Rust
   Answer: B — GC languages must ship a runtime into the module (or wait for WasmGC); Rust's runtime is nearly nothing. Same code, native and browser — that's how this site's simulators run identical logic everywhere.

**Q3. The main performance trap in JS↔Wasm apps is…**
   A. Wasm instructions are 10× slower than native
   B. Crossing the boundary too often: marshalling strings/objects per call dominates — the fix is rare crossings with big payloads
   C. Linear memory is slower than DOM memory
   D. Browsers throttle Wasm after 30 seconds
   Answer: B — Compute inside Wasm is near-native; the toll booth is the boundary (same law as JNI, C extensions, and CPU↔GPU transfers). Batch across it; stay on one side per frame.

**Q4. Server-side Wasm (wasmtime/WASI/Workers) sells which property over containers?**
   A. Bigger memory limits
   B. Microsecond cold starts and structural sandboxing for untrusted/plugin code — isolation without an OS process per unit
   C. Free GPU access
   D. Automatic scaling to zero cost
   Answer: B — No process, no image pull: instantiate a module in µs with capability security. That is why it wins plugin systems and edge functions — and why your security team prefers it to customer-supplied .so files.
