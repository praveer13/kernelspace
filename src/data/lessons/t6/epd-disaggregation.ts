...
Disaggregation lives or dies on KV transfer (T5.L8's ~25 ms budget line). Two artifacts turned that from a custom project into plumbing:
...
**llm-d** is the packaging answer for teams that don't want to assemble this: Kubernetes-native disaggregated serving — prefill/decode worker pools, KV-aware routing, Mooncake-style cache tiers, autoscalers that understand TTFT. If Dynamo is NVIDIA's fabric, llm-d is the cloud-native distribution. For your career: the job posts say "Kubernetes + disaggregated inference" and mean this stack.`,
    ...
    },
    {
      ...
      md: `The 2025-26 evidence didn't repeal T5.L8's rule — it sharpened it. Disaggregation wins when: contexts are long (KV transfer amortizes), prefill/decode ratios are skewed, and the fleet is big enough to bin-pack phases (DeepSeek's 200+ nodes, Kimi's 128). It loses when: deployment is small, contexts are short, or traffic is uniform. The honest decision procedure: compute the KV-transfer budget per request (T5.L8's ~25 ms for 70B 4k over 50 GB/s), compare to the ITL isolation win, and remember that an idle prefill GPU is still a cost. Every "always disaggregate" claim is marketing; every "never disaggregate" claim is a small fleet talking.`,
    },
  ],
}
